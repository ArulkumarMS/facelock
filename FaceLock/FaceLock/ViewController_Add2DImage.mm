...
@interface ViewController_Add2DImage (){
}

@end

@implementation ViewController_Add2DImage
    ...
    if (_count == 1) {
        ...
        dispatch_async(face_recognition_queue, ^{
            ...
            for(int i =0; i <_faces.size(); i++){
                NSLog(@"image count is %d",_imagename_count);
                if (_imagename_count<=50) {
                    NSString* imagename = [NSString stringWithFormat:@"%@2D%d.jpg", self.UserName, _imagename_count];
                    
                    while ([Setting_ImageManagement ImageExist:imagename]) {
                        _imagename_count++;
                        imagename = [NSString stringWithFormat:@"%@2D%d.jpg", self.UserName, _imagename_count];
                    }
