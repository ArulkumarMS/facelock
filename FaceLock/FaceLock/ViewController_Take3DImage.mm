...
@interface ViewController_Take3DImage () <AVCaptureVideoDataOutputSampleBufferDelegate> {
...
-(BOOL) faceSegmentation:(cv::Mat&) depth_mat faceRect:(cv::Rect&) face faceMat:(cv::Mat&) face_mat
{
    ...
    const double MAX_FACE_DEPTH = 96;
...
//        cv::Mat depth_mat = cv::Mat((int)rows, (int)cols, CV_32FC1, (void*)_floatDepthFrame.depthAsMillimeters);
        // Face Recognition
        cv::Rect face;
        cv::Mat face_mat;
